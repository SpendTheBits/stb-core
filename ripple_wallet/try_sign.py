...
class Seed:
    ...
    def encode_base58(self):
        ...
        string such as ''.
        """
        # logger.info(XRPL_SEED_PREFIX)
        # logger.info((self.bytes))
        arr = bytearray(XRPL_SEED_PREFIX)
        arr+=bytearray(self.bytes)
        # logger.info(arr)
        # print (XRPL_SEED_PREFIX+ self.bytes)
        return base58.b58encode_check(XRPL_SEED_PREFIX + self.bytes).decode()
        #return base58.b58encode_check(XRPL_SEED_PREFIX).decode()
        #return base58.b58encode_check(self.bytes).decode()
    ...
    def encode_ed25519_public_base58(self):
        ...
        return base58.b58encode_check(prefix +
                                      b"lol").decode()#self.ed25519_public_key).decode()
...
def get_byte_key(phrase):

    seed = Seed(phrase)
    seed.derive_secp256k1_master_keys()
    return seed.bytes

def get_mnemonic_code(phrase):

    seed = Seed(phrase)
    j = seed.encode_rfc1751()
    logger.info("j is ",j)
   
    hex = seed.encode_hex()
    b58 = base58.b58encode(hex)
